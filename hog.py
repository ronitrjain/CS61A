...
def max_scoring_num_rolls(dice=six_sided, total_samples=1000):
    ...
    n = 1
    while x < 11:
        avg = make_averaged(roll_dice, total_samples)(x, dice)
        if  avg > max:
            max = avg
            n = x
        x += 1
...
def square_strategy(score, opponent_score, threshold=12, num_rolls=6):
    ...
    tp = tail_points(opponent_score)
    if tp >= threshold:
        return 0
    elif perfect_square(tp + score) and next_perfect_square(tp + score) - score > threshold:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
