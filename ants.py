...
class Place:
    ...
    def add_insect(self, insect):
        """
        Asks the insect to add itself to the current places. This method exists so
...
# END Problem 12
    ...
    def action(self, gamestate):
        ...
        # BEGIN Problem 12
        super().action(gamestate)
        ants_behind = self.place.exit
        while ants_behind: 
            if ants_behind.ant is not None: 
                if ants_behind.ant.is_container and ants_behind.ant.ant_contained is not None: 
                    if not ants_behind.ant.ant_contained.is_doubled: 
                        ants_behind.ant.ant_contained.double()
                        ants_behind.ant.ant_contained.is_doubled = True
                if not ants_behind.ant.is_doubled: 
                    ants_behind.ant.double()
                    ants_behind.ant.is_doubled = True 
                
            ants_behind = ants_behind.exit
    def remove_from(self, place): 
        return None 
...
class SlowThrower(ThrowerAnt):
    ...
    def helper(self, gamestate): 
        if self.turns_till_action == 0: 
            self.action = Bee.action.__get__(self, self.__class__)
            self.action(gamestate)
        else: 
            if gamestate.time % 2 == 0: 
                Bee.action(self, gamestate)
            self.turns_till_action -= 1
    def throw_at(self, target): 
        if target is not None: 
            target.turns_till_action = 5
            target.action = SlowThrower.helper.__get__(target, target.__class__)
