...
def about(topic):
    ...
    def helper(paragraph): 
        ...
        for a in range(len(topic)): 
            for b in range(len(paragraph)): 
                w1 = lower(remove_punctuation(topic[a]))
                w2 = lower(remove_punctuation(paragraph[b]))
                if w1 == w2: 
                    return True 
        return False 
    return helper
    ...
    # END PROBLEM 2
print(about(['dogs', 'hounds'])('A paragraph about dogs'))
...
def accuracy(typed, source):
    ...
    # BEGIN PROBLEM 3
    same_words = 0
    for typed_word in range(min(len(typed_words), len(source_words))): 
        if typed_words[typed_word] == source_words[typed_word]:
            same_words += 1 
    if len(typed_words) == 0 and len(source_words) == 0: 
        return 100.0
    if len(typed_words) == 0 or len(source_words) == 0: 
        return 0.0
    return same_words / len(typed_words) * 100
    # END PROBLEM 3
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    if typed_word in word_list: 
        ...
        i = min(diff_function(typed_word, word, limit) for word in word_list)
        for word in word_list: 
            if i > limit: 
                return typed_word
            if diff_function(typed_word, word, limit) == i: 
                return word
    # END PROBLEM 5
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    mylist = []
    player_times = get_all_times(match)
    for player_index in player_indices: 
        list2 = []
        for n in range(len(player_times[0])): 
            min = 0
            for m in range(len(player_times)): 
                if player_times[m][n] < player_times[min][n]:
                    min = m 
            if min == player_index: 
                list2.append(get_all_words(match)[n])
        mylist.append(list2)
    return mylist
